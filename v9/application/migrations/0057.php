<?php
class Migration_57 extends Migration {

    const DESCRIPTION_RU = 'Создание таблицы index_os';

    const DESCRIPTION_EN = 'Create table index_os';

    public static function up()
    {
        $table = self::getDb()->getPrefix() . 'index_os';
        $oldTable = self::getDb()->getPrefix() . 'archive_os';
        ...
        $sql = 'INSERT INTO ' . $table. ' (campaign_id, date, value, `count`, `unique`)
            SELECT group_id, date, os, `count`, `count` FROM ' . $oldTable .'';
        self::execute($sql);
    }
}