/*
  ...
  After changes you must minify script to k.min.js
*/
KTracking = KTracking || {listeners: []};
KTracking.DEBUG = false;
...
KTracking.getSubId = function(cb) {
  ...
  this.multiDomain = this['multiDomain'] || false;
...
KTracking.updateLinks = function(subId, token) {
  ...
  for (var i = 0; i < elms.length; i++) {
    ...
    if (elms[i].href && (elms[i].getAttribute("href").indexOf('subid') !== -1 || elms[i].getAttribute("href").indexOf('sub_id') !== -1)) {
        elms[i].href = elms[i].getAttribute("href")
            .replace('{subid}', subId)
            .replace('%7Bsubid%7D', subId)
            .replace('{sub_id}', subId)
            .replace('%7Bsub_id%7D', subId);
    }
...
KTracking.updateForms = function(subId, token) {
    ...
    for (var i = 0; i < forms.length; i++) {
        forms[i].appendChild(subIdInput.cloneNode(true));
        forms[i].appendChild(tokenInput.cloneNode(true));
    }
};
